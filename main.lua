...
-- Nuevos sonidos atmosféricos tipo ráfaga/viento suave digital
local function PlayWindToggleOn() PlaySound(134012322, 0.4, 1.2) end   -- Viento rápido agudo
local function PlayWindToggleOff() PlaySound(134012322, 0.35, 0.8) end -- Viento rápido grave
local function PlayWindMenuOpen() PlaySound(9114228358, 0.5, 0.9) end  -- Ráfaga de apertura profunda
local function PlayWindMenuClose() PlaySound(9114228358, 0.4, 1.1) end -- Ráfaga de cierre rápido
...
local function Notify(msg, color)
    ...
    task.delay(3, function()
        FastTween(Frame, {Position = UDim2.new(1, 30, 0.9, 0)}, 0.4)
        task.wait(0.4)
        NotifyGui:Destroy()
    end)
end
...
-- ==================== MOTOR DE ESP TOTALMENTE RECONSTRUIDO (ANTI-CONGELADO) ====================
local ESPObjects = {}
...
local function CreateESPObjects(player)
    RemoveESP(player) -- Evitar duplicados fantasmas
    ...
    obj.Box.Thickness = 1.8; obj.Box.Color = SoftRedESP
...
local function HideESP(objs)
    if objs then
        objs.Name.Visible = false; objs.DistLabel.Visible = false; objs.Box.Visible = false
...
RunService.RenderStepped:Connect(function()
    for _, p in pairs(Players:GetPlayers()) do
        ...
        -- Si la opción global está apagada o el jugador ya no es válido, ocultar inmediatamente
        if not Settings.ESP or not char or not char:FindFirstChild("Humanoid") or char.Humanoid.Health <= 0 or not char:FindFirstChild("HumanoidRootPart") then
        ...
        if onScreen then
            ...
            objs.Box.Color = isWhitelisted and Color_NeonBlue or SoftRedESP
            ...
            objs.DistLabel.Position = Vector2.new(screenPos.X, y + h + 6)
    ...
    -- Limpieza preventiva de tracking obsoletos
...
-- Capa de Partículas Súper Visibles
...
LeftFrame.Size = UDim2.new(0.48, 0, 0.82, 0)
LeftFrame.Position = UDim2.new(0.03, 0, 0.15, 0)
LeftFrame.BackgroundTransparency = 1

local LeftLayout = Instance.new("UIListLayout", LeftFrame)
LeftLayout.Padding = UDim.new(0, 8); LeftLayout.SortOrder = Enum.SortOrder.LayoutOrder

-- Slider FOV Card
local FOVCard = Instance.new("Frame", LeftFrame)
FOVCard.Size = UDim2.new(1, 0, 0, 70)
...
FOVLabel.Size = UDim2.new(1, -30, 0, 30); FOVLabel.Position = UDim2.new(0, 15, 0, 6)
...
SliderBar.Size = UDim2.new(0.9, 0, 0, 6); SliderBar.Position = UDim2.new(0.05, 0, 0.68, 0)
...
AimPartBtn.Size = UDim2.new(1, 0, 0, 44); AimPartBtn.BackgroundColor3 = Color_Card; AimPartBtn.BorderSizePixel = 0
...
local function AddToggle(name, key, order)
    ...
    btn.Size = UDim2.new(1, 0, 0, 44); btn.BorderSizePixel = 0; btn.Text = "    " .. name
    ...
    local function updateVisuals(isInitial)
        if not isInitial then
            -- Sonido de ráfagas suaves modificado
...
AddToggle("Show HP Bar", "HPBar", 6)
AddToggle("Hide My Name", "HideName", 7)
...
local function UpdateWhitelistMenu()
    ...
    for _, p in pairs(Players:GetPlayers()) do
        ...
        btn.MouseButton1Click:Connect(function()
            ...
            Notify(Settings.Whitelist[p.Name] and "Añadido: " .. p.DisplayName or "Removido: " .. p.DisplayName)
            UpdateWhitelistMenu()
        end)
    end
end
...
ToggleBtn.MouseButton1Click:Connect(function()
    ...
    if Main.Visible then
        ...
        PlayWindMenuOpen()
        FastTween(Main, {Size = UDim2.new(0, 660, 0, 500)}, 0.45, Enum.EasingStyle.Back)
        UpdateWhitelistMenu() 
        task.wait(0.4)
        isMenuMoving = false
    end
...
Notify("Piru HUB | Corrección Estática Lista", Color_NeonBlue)
